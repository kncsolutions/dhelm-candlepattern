...
/**
*According to literature the descending hawk is a two line pattern composed of a
*long candle with white body followed by a  white candle, where the body of the second candle is fully engulfed by the body of the 
previous candle. The previous trend should be up trend
*/
public class DescendingHawk{
...
private boolean isDescendingHawk;
/**
...
*/
public DescendingHawk(List<Double> open,List<Double> high,List<Double> low,List<Double> close,int numBody,int numLen,int numTrend,double percentage)throws DataException{
  Open.addAll(open);
  High.addAll(high);
  Low.addAll(low);
  Close.addAll(close);
...
*
*/
private void DescendingHawk()throws DataException{
...
private boolean validBasicCandle(int i)throws DataException{
   if(i==1 && (new ShortWhiteCandle(Open,High,Low,Close,RefBody,RefLength,Percentage)).isShortWhiteCandle()
   ||(new WhiteCandle(Open,High,Low,Close,RefBody,RefLength,Percentage)).isWhiteCandle()
...
/**
*@return Returns true if the descending hawk pattern is generated. 
*/
public boolean isDescendingHawk(){
  return isDescendingHawk;
}
}