...
/**
*According to literature the bullish harami pattern is a two line pattern composed of a
*long candle with black body followed by a white candle, where the body of the white candle is fully engulfed 
*by the body of the previous black candle. The prior trend should be down trend.
*/
public class BullishHarami{
...
private boolean isBullishHarami;
/**
...
*/
public BullishHarami(List<Double> open,List<Double> high,List<Double> low,List<Double> close,int numBody,int numLen,int numTrend,double percentage)throws DataException{
  Open.addAll(open);
  High.addAll(high);
  Low.addAll(low);
  Close.addAll(close);
...
*
*/
private void BullishHarami()throws DataException{
  if((Close.get(0)>=Open.get(0)) && !((new FourPriceDoji(Open,High,Low,Close)).isFourPriceDoji())){
