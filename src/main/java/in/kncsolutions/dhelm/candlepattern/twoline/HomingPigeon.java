...
/**
*According to literature the homing pigeon is a two line pattern composed of a
*long candle with black body followed by a  black candle, where the body of the second candle is fully engulfed by the body of the 
previous candle. The previous trend should be down trend
*/
...
private boolean isHomingPigeon;
/**
...
*/
public HomingPigeon(List<Double> open,List<Double> high,List<Double> low,List<Double> close,int numBody,int numLen,int numTrend,double percentage)throws DataException{
  Open.addAll(open);
  High.addAll(high);
  Low.addAll(low);
  Close.addAll(close);
...
*
*/
private void HomingPigeon()throws DataException{
  if((Close.get(0)<Open.get(0)) && validBasicCandle(1)){
    if((Close.get(1)<Open.get(1)) && validBasicCandle(2)){
	 if(Open.get(0)<Open.get(1) && Close.get(0)>Close.get(1)){
...
private boolean validBasicCandle(int i)throws DataException{
   if(i==1 && (new ShortBlackCandle(Open,High,Low,Close,RefBody,RefLength,Percentage)).isShortBlackCandle()
   ||(new BlackCandle(Open,High,Low,Close,RefBody,RefLength,Percentage)).isBlackCandle()
  ...
  else if(i==2 && (new BlackCandle(Open.subList(1,Open.size()),High.subList(1,High.size()),Low.subList(1,Low.size()),Close.subList(1,Close.size()),RefBody,RefLength,Percentage)).isBlackCandle()
...
/**
*@return Returns true if the homing pigeon pattern is generated. 
*/
public boolean isHomingPigeon(){
