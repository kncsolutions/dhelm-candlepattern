...
/**
*According to literature the bearish harami cross pattern is a two line pattern composed of a
*long candle with white body followed by a doji candle with both shadows, where the doji candle along with both the shadows is fully engulfed 
*by the body of the previous white candle. The prior trend should be down trend.
*/
...
private boolean isBearishHaramiCross;
/**
...
*@param low : List of low prices where fisrt element is the latest data.
...
*/
public BearishHaramiCross(List<Double> open,List<Double> high,List<Double> low,List<Double> close,int numBody,int numLen,int numTrend,double percentage)throws DataException{
  Open.addAll(open);
  High.addAll(high);
  Low.addAll(low);
  Close.addAll(close);
...
/**
*@return Returns true if the bearish harami cross pattern is generated. 
*/
public boolean isBearishHaramiCross(){
  return isBearishHaramiCross;
}
}