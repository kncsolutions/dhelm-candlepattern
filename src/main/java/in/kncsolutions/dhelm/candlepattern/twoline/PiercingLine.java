...
/**
*According to literature the piercing line is a two line pattern composed of a
*long candle with black body followed by a long white candle, where the open of the white candle is below or equal to the closing level of the black candle. 
*The closing of the white candle is in between the mid point of the body and opening price of the black candle. The prior trend should be down trend.
*/
...
private boolean isPiercingLine;
/**
...
*/
public PiercingLine(List<Double> open,List<Double> high,List<Double> low,List<Double> close,int numBody,int numLen,int numTrend,double percentage)throws DataException{
  Open.addAll(open);
  High.addAll(high);
  Low.addAll(low);
  Close.addAll(close);
...
*
*/
private void PiercingLine()throws DataException{
  if((Close.get(0)>Open.get(0)) && validBasicCandle(2)){
    if((Close.get(1)<Open.get(1)) && validBasicCandle(1)){
...
private boolean validBasicCandle(int i)throws DataException{
   if(i==1 && (new BlackCandle(Open.subList(1,Open.size()),High.subList(1,High.size()),Low.subList(1,Low.size()),Close.subList(1,Close.size()),RefBody,RefLength,Percentage)).isBlackCandle()
  ...
  else if(i==2 && (new WhiteCandle(Open,High,Low,Close,RefBody,RefLength,Percentage)).isWhiteCandle()
...
/**
*@return Returns true if the piercing line pattern is generated. 
*/
public boolean isPiercingLine(){
  return isPiercingLine;
}
}