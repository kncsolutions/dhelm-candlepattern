...
import in.kncsolutions.dhelm.candlepattern.twoline.BearishEngulfing;
/**
*According to literature the three outside down  pattern is a three line pattern composed of a
*bearish engulfing pattern followed by a black candle , where the closing of the  black candle is below the closing of the
*second candle(which is a black candle) of the previous bearish engulfing pattern.
*/
public class ThreeOutsideDown{
...
private boolean isThreeOutsideDown;
/**
...
*/
public ThreeOutsideDown(List<Double> open,List<Double> high,List<Double> low,List<Double> close,int numBody,int numLen,int numTrend,double percentage)throws DataException{
  Open.addAll(open);
  High.addAll(high);
  Low.addAll(low);
  Close.addAll(close);
...
*
*/
private void ThreeOutsideDown()throws DataException{
...
if((new BearishEngulfing(Open.subList(1,Open.size()),High.subList(1,High.size()),Low.subList(1,Low.size()),Close.subList(1,Close.size()),RefBody,RefLength,RefTrend,Percentage)).isBearishEngulfing()){
  return true;
}
  return false;
}
/**
*@return Returns true if the three outside down pattern is generated. 
*/
public boolean isThreeOutsideDown(){
  return isThreeOutsideDown;
}
}
