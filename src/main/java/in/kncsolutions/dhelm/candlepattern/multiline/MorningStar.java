...
/**
*According to literature the morning  star  pattern is a three line pattern composed of a
*long candle with black body followed by a short candle with with either white or black body, followed by a long white candle, where the body of the middle short candle is below the closing of the
*previous black candle and the following white candle's opening is above the body of the middle candle. The closing of the white candle should be above the mid point of the body of the 
...
*/
public MorningStar(List<Double> open,List<Double> high,List<Double> low,List<Double> close,int numBody,int numLen,int numTrend,double percentage)throws DataException{
  Open.addAll(open);
  High.addAll(high);
  Low.addAll(low);
  Close.addAll(close);
...
*
*/
private void MorningStar()throws DataException{
  if((Close.get(0)>Open.get(0)) && validBasicCandle(1)){
    if((Close.get(1)!=Open.get(1)) && isShort()){
	  if((Close.get(2)<Open.get(2)) && validBasicCandle(3)){
	    if(Open.get(0)>Math.max(Close.get(1),Open.get(1)) && Math.max(Close.get(1),Open.get(1))<Close.get(2) && Close.get(0)>(Open.get(2)+Close.get(2))/2){
...
/**
*@return Returns true if the morning  star pattern is generated. 
*/
