...
/**
*According to literature the evening doji star  pattern is a three line pattern composed of a
*long candle with white body followed by a doji candle with at least one shadow, followed by a long black candle, where the closing of the  doji candle is above the closing of the
*previous white candle and the following black candle's opening is lower than the closing of the doji candle. The closing of the black candle should be below the mid point of the body of the 
...
private boolean isEveningDojiStar;
/**
...
*/
public EveningDojiStar(List<Double> open,List<Double> high,List<Double> low,List<Double> close,int numBody,int numLen,int numTrend,double percentage)throws DataException{
  Open.addAll(open);
  High.addAll(high);
  Low.addAll(low);
  Close.addAll(close);
...
*
*/
private void EveningDojiStar()throws DataException{
...
if( !(new BearishAbandonedBaby(Open,High,Low,Close,RefBody,RefLength,RefTrend,Percentage)).isBearishAbandonedBaby())return true;
return false;
}
/**
*@return Returns true if the evening doji star pattern is generated. 
*/
public boolean isEveningDojiStar(){
