...
/**
*According to literature the bullish side by side white lines  pattern is a three line pattern composed of a
*long candle with white body followed by two white candles, where the low of the latter two white candles are above the high of the first long white candle
*and opening and closing of the latter two candles are similar.
*/
public class BullishSidebySideWhiteLines{
...
private boolean isBullishSidebySideWhiteLines;
/**
...
*/
public BullishSidebySideWhiteLines(List<Double> open,List<Double> high,List<Double> low,List<Double> close,int numBody,int numLen,int numTrend,double percentage)throws DataException{
  Open.addAll(open);
  High.addAll(high);
  Low.addAll(low);
  Close.addAll(close);
  ...
  isBullishSidebySideWhiteLines=false;
...
*
*/
private void BullishSidebySideWhiteLines()throws DataException{
  if((Close.get(2)>Open.get(2)) && validBasicCandle(3)){
    if((Close.get(1)>Open.get(1)) && Low.get(1)>High.get(2)){
	  if((Close.get(0)>Open.get(0)) && Low.get(0)>High.get(2)){
        if(Math.abs(Close.get(1)-Close.get(0))/Math.min(Close.get(1),Close.get(0))<1 && Math.abs(Open.get(1)-Open.get(0))/Math.min(Open.get(1),Open.get(0))<1){	    
	      ...
	      if(trendPrior[0]<0)
		    isBullishSidebySideWhiteLines=true;
		}
	  }
	}
  }
...
/**
*@return Returns true if the bullish side by side white lines pattern is generated. 
*/
public boolean isBullishSidebySideWhiteLines(){
  return isBullishSidebySideWhiteLines;
}
}