...
/**
*According to literature the three inside down  pattern is a three line pattern composed of a
*bearish harami pattern followed by a black candle , where the closing of the  latest black candle is below the closing of the
*second candle(which is a black candle) of the previous bearish harami pattern.
*/
...
if((new BearishHarami(Open.subList(1,Open.size()),High.subList(1,High.size()),Low.subList(1,Low.size()),Close.subList(1,Close.size()),RefBody,RefLength,RefTrend,Percentage)).isBearishHarami()){
  return true;
}
  return false;
}
/**
*@return Returns true if the three inside down pattern is generated. 
*/
public boolean isThreeInsideDown(){
  return isThreeInsideDown;
}
}