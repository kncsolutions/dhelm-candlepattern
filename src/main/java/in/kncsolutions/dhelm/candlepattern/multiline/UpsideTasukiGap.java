...
*/
public UpsideTasukiGap(List<Double> open,List<Double> high,List<Double> low,List<Double> close,int numBody,int numLen,int numTrend,double percentage)throws DataException{
  Open.addAll(open);
  High.addAll(high);
  Low.addAll(low);
  Close.addAll(close);
...
private void UpsideTasukiGap()throws DataException{
  if((Close.get(2)>Open.get(2)) && validBasicCandle(3)){
    if((Close.get(1)>Open.get(1)) && Low.get(1)>High.get(2)){
	  if((Close.get(0)<Open.get(0)) && Open.get(0)<=Close.get(1) && Open.get(0)>=Open.get(1) && Low.get(0)>High.get(2)){	    
