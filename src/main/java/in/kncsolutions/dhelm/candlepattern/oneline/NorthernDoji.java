 ...
 */
 ...
 /**
 *According to literature the northern doji pattern is a single line pattern composed of
 *a  doji candle. The candle can be any doji candle except four price doji. The prior trend should be up trend.
 */
 public class NorthernDoji{
   ...
   private boolean isNorthernDoji;   
   /**
  ...
  */
   public NorthernDoji(List<Double> open,List<Double> high,List<Double> low,List<Double> close,int numTrend)throws DataException{
    Open.addAll(open);
	 High.addAll(high);
	 Low.addAll(low);
	 Close.addAll(close);
	 RefTrend=numTrend;
	 isNorthernDoji=false;
   ...
   */
   private void NorthernDoji(){
     if((Open.get(0)==Close.get(0))
	    ...
	    if(trendPrior[0]<0)isNorthernDoji=true;
	 }
   }
   /**
   *@return Returns true if the Northern doji pattern is generated. 
   */
   public boolean isNorthernDoji(){
     return isNorthernDoji;
   }
 }