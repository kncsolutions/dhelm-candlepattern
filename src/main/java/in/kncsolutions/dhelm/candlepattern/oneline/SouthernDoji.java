 ...
 */
 ...
 /**
 *According to literature the southern doji pattern is a single line pattern composed of
 *a  doji candle. The candle can be any doji candle except four price doji. The prior trend should be down trend.
 */
 public class SouthernDoji{
   ...
   private boolean isSouthernDoji;   
   /**
  ...
  */
   public SouthernDoji(List<Double> open,List<Double> high,List<Double> low,List<Double> close,int numTrend)throws DataException{
     Open.addAll(open);
	 High.addAll(high);
	 Low.addAll(low);
	 Close.addAll(close);
	 RefTrend=numTrend;
	 isSouthernDoji=false;
	 if(Open.size()<(RefTrend+1)
   ...
   */
   private void SouthernDoji(){
     if((Open.get(0)==Close.get(0))
	    ...
	    if(trendPrior[0]>0)isSouthernDoji=true;
	 }
   }
   /**
   *@return Returns true if the southern doji pattern is generated. 
   */
   public boolean isSouthernDoji(){
     return isSouthernDoji;
   }
 }