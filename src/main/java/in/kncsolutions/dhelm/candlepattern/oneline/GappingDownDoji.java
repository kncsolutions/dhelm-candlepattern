 ...
 */
 ...
 /**
 *According to literature the gapping Down pattern is a single line pattern composed of
 *a  doji candle. The candle can be any doji candle except four price doji. The prior trend should be Down trend.
 *The high of the doji candle should be lower than the low of the previous candle.
 */
 public class GappingDownDoji{
   ...
   private boolean isGappingDownDoji;   
   /**
  ...
  */
   public GappingDownDoji(List<Double> open,List<Double> high,List<Double> low,List<Double> close,int numTrend)throws DataException{
     Open.addAll(open);
	 High.addAll(high);
	 Low.addAll(low);
	 Close.addAll(close);
	 RefTrend=numTrend;
	 isGappingDownDoji=false;
   ...
   */
   private void GappingDownDoji(){
     if((Open.get(0)==Close.get(0))
	    ...
	    if(trendPrior[0]>0 && High.get(0)<Low.get(1)){
		  isGappingDownDoji=true;
		}
	 }
   }
   /**
   *@return Returns true if the Gapping Down doji pattern is generated. 
   */
   public boolean isGappingDownDoji(){
     return isGappingDownDoji;
   }
 }